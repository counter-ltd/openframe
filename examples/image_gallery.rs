...
impl ImageCacheProvider for SimpleLruCacheProvider {
    fn provide(&mut self, window: &mut Window, cx: &mut App) -> openframe::AnyImageCache {
        window
...
struct SimpleLruCache {
    max_items: usize,
    usages: Vec<u64>,
    cache: HashMap<u64, openframe::ImageCacheItem>,
}
...
impl ImageCache for SimpleLruCache {
    fn load(
        &mut self,
        resource: &openframe::Resource,
        window: &mut Window,
        cx: &mut App,
    ) -> Option<Result<Arc<openframe::RenderImage>, openframe::ImageCacheError>> {
        ...
        self.cache
            .insert(hash, openframe::ImageCacheItem::Loading(task.clone()));
        self.usages.insert(0, hash);
